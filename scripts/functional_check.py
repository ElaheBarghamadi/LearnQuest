...
def main():
    ...
    r = s.get(BASE + '/academy/lesson/1/', timeout=30)
    ok(r.status_code == 200, 'lesson page')
    r = post_json(s, '/academy/api/update-progress/1/', {'progress': 50})
    ok(r.status_code in (200, 400), 'update progress', r.text[:150])

    print('=== Academy: vocabulary actions ===')
    r = post_json(s, '/academy/vocabulary/mark-learned/1/', {})
    ok(r.status_code == 200, 'vocab mark learned', r.text[:150])
    r = post_json(s, '/academy/vocabulary/flashcard-action/', {'word_id': 1, 'action': 'known'})
    ok(r.status_code == 200, 'flashcard action', r.text[:150])
    r = post_json(s, '/academy/vocabulary/spaced-repetition-action/', {'word_id': 1, 'quality': 4})
    ok(r.status_code == 200, 'spaced repetition action', r.text[:150])
    r = post_json(s, '/academy/vocabulary/matching-result/', {'word_id': 1, 'is_correct': True})
    ok(r.status_code == 200, 'matching result', r.text[:150])
    r = post_json(s, '/academy/vocabulary/add-to-practice/2/', {})
